...
# SAMPLE local config , to directly let RBLDNSD startup
test -d $rbldnshome && mkdir -p $rbldnshome/local; chown rbldns:rbldns $rbldnshome/local
test -f  $rbldnshome/local/local.txt &&  cat >$rbldnshome/local/list.txt << EOF
;
; SAMPLE local config , to directly let RBLDNSD startup
;
cn :2: All subdomains of .com.cn lack security administrators - allmost intruders
hk :2: All subdomains of .com.cn lack security administrators - allmost intruders
com.br :2: All subdomains of .com.cn lack security administrators - allmost intruders
com.cn :2: All subdomains of .com.cn lack security administrators - allmost intruders
;
...
#
#  list.dsbl.org on local 127.0.0.10 port 253
RBLDNSD="dsbl -r/var/lib/rbldnsd/local -b127.10/253 -u rbldns -4 \
	 list.localhost:ip4set:list.txt "
#
EOF
#
...
%description local
rbldnsd - cron mirror jobs


%post local
#
cat <(crontab -l -u rbldns) <(echo "10,40 * * * * /var/lib/rbldnsd/bin/local.cron") | crontab -

# listening local on port 754
test -d /etc/rbldnsd && cat > /etc/rbldnsd/bl.remsnet.dyndns.org.zone << EOF
;
; bind9 zone config
zone "bl.remsnet.dyndns.org" IN {
        type forward;
        forward first;
        forwarders {
        127.0.0.1 port 853;
...
%files local
%defattr(0750,root,root,0750)
/var/lib/rbldnsd/bin/local.cron

%postun local
( crontab -l -u rbldns | egrep -v "" ) | crontab -u rbldns -
...
%post psbl
cat <(crontab -l -u rbldns) <(echo "10,40 * * * * /var/lib/rbldnsd/bin/psbl.cron") | crontab -
# listening local on port 754
test -d /etc/rbldnsd && echo "" >>/etc/rbldnsd/psbl.conf
test -d /etc/rbldnsd && echo RBLDNSD="dsbl -r/var/lib/rbldnsd/dsbl -b 127.3/453 psbl.org:ip4set:psbl.txt " >>/etc/rbldnsd/psbl.conf

test -d /etc/rbldnsd && cat > /etc/rbldnsd/bl.psbl.org-named.zone << EOF
;
; bind9 zone config
zone "bl.psbl.org" IN {
        type forward;
...
%postun psbl
( crontab -l -u rbldns | egrep -v "psbl.cron" ) | crontab -u rbldns -


%files psbl
%defattr(0750,root,root,0750)
/var/lib/rbldnsd/bin/psbl.cron
...
%changelog
* Sat May  3 2014 support@remsnet.de -r8
- moved psbl to %psbl subpackage
- added local as default with samples on port 253 
- added %local subpack with solution from http://www.blue-quartz.com/rbl/ to manage the local list
...
- added http://psbl.org/ as inital default list 
- added %suburl package - http://www.surbl.org/setup-local-rbl-mirror
